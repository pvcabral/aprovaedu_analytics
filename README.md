...
Ao rodar o `main.py`, cada tabela final é exportada para `data/tratados/<tabela>.csv`,
com o conteúdo exatamente como ficou no banco, ou seja, já com o tratamento, os
filtros de FK e os cadastros de estudante gerados na carga. Esse é o artefato de
base tratada, disponível mesmo sem subir o Postgres.

## Decisões técnicas relevantes

**Modelagem em Postgres com PK e FK.** A base foi modelada como um esquema
relacional (ver `src/schema.sql`), com uma dimensão de matérias (`dim_materia`)
gerada no tratamento e um relacionamento N:N `professor_materia`. Toda coluna que
referencia matéria usa o mesmo nome `materia_id` em todas as tabelas, para deixar
explícito que é FK.

**A base é uma amostra, não a base completa.** A planilha traz uma amostra: a
maioria das tabelas vem cortada em cerca de 500 linhas, e cada aba foi amostrada
de forma independente. Por isso alguns cruzamentos entre tabelas têm poucas
dezenas de registros, e alguns recortes cobrem só parte da realidade (os
resultados de simulado, por exemplo, só existem para matemática e física, e a
presença em aula só cobre 2021). As análises tratam isso de forma explícita.

**Estudantes stub para preservar dados linkáveis.** Como a amostra de estudantes
é incompleta, aprovações, presenças e resultados de simulado de alunos fora dessa
amostra seriam descartados pela FK com `estudantes`. Para não perder esse dado, o
`main.py` cria cadastros mínimos (stub, com `nome_aluno = 'cadastro ausente'`)
para todo `aluno_id` que aparece nos fatos mas não no cadastro. Isso mantém a
integridade referencial e ainda permite medir quantos alunos faltam no cadastro
para aquela métrica.
...
limite da escala (100) são capadas no próprio limite, para não perder o registro
do aluno nem deixar um valor extremo inflar a média.
...
casamento é feito pelo mesmo ano, para não comparar um evento de um ano com outro
de um ano diferente.

## Decisões analíticas (resumo)

As perguntas obrigatórias foram respondidas sempre sobre os dados processados, e
